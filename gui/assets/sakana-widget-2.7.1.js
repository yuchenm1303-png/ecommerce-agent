(function (global, factory) {
    ...
    class SakanaWidget {
        ...
        _run = () => {
            ...
            // Normalize every part of the spring integration to elapsed time.
            // Upstream only scales inertia below 60 fps; acceleration and damping
            // remain per-frame, which makes unlocked Chromium run too fast and
            // jitter indefinitely on high-refresh displays.
            const frameScale = Math.min(2, Math.max(0.25, lastRunUnixDiff / this._frameUnix));
            this._lastRunUnix = thisRunUnix;
            w = w - (r * 2 + originRotate) * frameScale;
            r = r + w * i * 1.2 * frameScale;
            this._state.w = w * Math.pow(d, frameScale);
            this._state.r = r;
            t = t - y * 2 * frameScale;
            y = y + t * i * 2 * frameScale;
            this._state.t = t * Math.pow(d, frameScale);
            this._state.y = y;
