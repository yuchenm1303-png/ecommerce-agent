...
async function resolveTenantScope(admin: ReturnType<typeof createClient>, userId: string): Promise<TenantScope> {
  const rows = await dataOrThrow(
    admin.rpc("get_listing_monitor_scope", { p_caller: userId }),
    "tenant_scope_failed",
  );
  const scopeRows = Array.isArray(rows) ? rows.map(objectValue) : [];
  if (!scopeRows.length) throw new Error("not_authorized");

  const first = scopeRows[0];
  const scopeKind = textValue(first.scope_kind) || "self";
  const tenantId = textValue(first.tenant_id);
  const tenantName = textValue(first.tenant_name) || "My Workspace";
  const tenantSlug = textValue(first.tenant_slug) || "self";
  const userIds = [...new Set(scopeRows.map((row) => textValue(row.user_id)).filter(Boolean))];
  ...
  return {
    ...
    tenantSlug,
    userIds,
    cacheKey: scopeKind === "tenant" && tenantId ? `tenant:${tenantId}` : `self:${userId}`,
  };
}

async function loadSnapshot(admin: ReturnType<typeof createClient>, userId: string, scope: TenantScope) {
  return await cached(`snapshot:${scope.cacheKey}`, TTL.snapshot, () =>
    dataOrThrow(admin.rpc("get_listing_usage_tenant_snapshot", { p_caller: userId }), "usage_snapshot_failed"));
}

async function loadTaskSummaries(admin: ReturnType<typeof createClient>, scope: TenantScope) {
  return await cached(`task-summaries:${scope.cacheKey}`, TTL.taskSummaries, async () => {
    const rows = await dataOrThrow(
...
async function loadTaskDetails(admin: ReturnType<typeof createClient>, scope: TenantScope) {
  return await cached(`task-details:${scope.cacheKey}`, TTL.taskDetails, async () => {
    const rows = await dataOrThrow(
      admin.from("listing_task_audits")
...
async function loadDiagnostics(admin: ReturnType<typeof createClient>, scope: TenantScope) {
  return await cached(`diagnostics:${scope.cacheKey}`, TTL.diagnostics, () =>
    dataOrThrow(
...
async function loadSystem(admin: ReturnType<typeof createClient>, userId: string, scope: TenantScope) {
  return await cached(`system:${scope.cacheKey}`, TTL.system, () =>
    dataOrThrow(admin.rpc("get_listing_usage_tenant_system_samples", {
      p_caller: userId,
...
async function loadHeatmap(admin: ReturnType<typeof createClient>, userId: string, scope: TenantScope) {
  return await cached(`heatmap:${scope.cacheKey}`, TTL.heatmap, () =>
    dataOrThrow(admin.rpc("get_listing_usage_tenant_daily_heatmap", {
      p_caller: userId,
      p_days: DAILY_HEATMAP_DAYS,
    }), "daily_heatmap_failed"));
}

function basePayload(snapshot: unknown, scope: TenantScope): JsonObject {
  const payload = snapshot && typeof snapshot === "object" && !Array.isArray(snapshot)
    ? { ...(snapshot as JsonObject) }
    : {};
  payload.query_architecture = "usage_monitor_tenant_read_model_v1";
  payload.viewer_scope = scope.scopeKind;
  payload.tenant = {
    id: scope.tenantId || null,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      return json(req, { error: error instanceof Error && error.message === "tenant_scope_failed" ? "tenant_scope_failed" : "not_authorized" }, 403);
    }
    ...
    if (scope === "task_detail") {
      ...
      const { data: row, error } = await admin.from("listing_task_audits")
        .select("id,user_id,device_id,app_version,task_kind,phase,status,product_url,input_data,result_data,error_text,started_at,completed_at,updated_at,created_at")
        .eq("id", sourceId)
        .in("user_id", tenantScope.userIds)
        .maybeSingle();
      ...
      return json(req, {
        query_architecture: "usage_monitor_tenant_read_model_v1",
        viewer_scope: tenantScope.scopeKind,
        tenant: { id: tenantScope.tenantId || null, name: tenantScope.tenantName, slug: tenantScope.tenantSlug },
    ...
    if (scope === "heatmap") {
      try {
        const heatmap = await loadHeatmap(admin, user.id, tenantScope);
        return json(req, {
          query_architecture: "usage_monitor_tenant_read_model_v1",
          viewer_scope: tenantScope.scopeKind,
          tenant: { id: tenantScope.tenantId || null, name: tenantScope.tenantName, slug: tenantScope.tenantSlug },
    ...
    let snapshot;
    try {
      snapshot = await loadSnapshot(admin, user.id, tenantScope);
    } catch {
      return json(req, { error: "usage_snapshot_failed" }, 503);
    }

    const payload = basePayload(snapshot.value, tenantScope);
    const partialErrors: JsonObject[] = [];
    if (snapshot.stale) partialErrors.push({ component: "summary", code: "stale_cache" });

    if (scope === "core") {
      try {
        const tasks = await loadTaskSummaries(admin, tenantScope);
        payload.task_audits = tasks.value;
        payload.task_audit_limit = TASK_AUDIT_LIMIT;
        payload.task_audit_basis = "tenant_lightweight_summary_lazy_detail";
    ...
    if (scope === "ops") {
      const [tasksResult, diagnosticsResult, systemResult] = await Promise.allSettled([
        loadTaskDetails(admin, tenantScope),
        loadDiagnostics(admin, tenantScope),
        loadSystem(admin, user.id, tenantScope),
      ]);
      ...
      payload.system_sample_basis = "tenant_persistent_5m_rollup_v1";
    ...
    const [tasksResult, diagnosticsResult, systemResult, heatmapResult] = await Promise.allSettled([
      ...
      loadDiagnostics(admin, tenantScope),
      loadSystem(admin, user.id, tenantScope),
      loadHeatmap(admin, user.id, tenantScope),
    ]);
    payload.task_audits = tasksResult.status === "fulfilled" ? tasksResult.value.value : [];
    payload.diagnostic_reports = diagnosticsResult.status === "fulfilled" ? diagnosticsResult.value.value : [];
    payload.system_samples = systemResult.status === "fulfilled" ? systemResult.value.value : [];
