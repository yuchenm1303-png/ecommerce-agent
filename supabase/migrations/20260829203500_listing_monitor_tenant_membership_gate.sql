...
as $function$
  ...
  member_tenant as (
    select c.id, c.name, c.slug
    from member_tenant_candidates c
    where (select count(*) from member_tenant_candidates) = 1
    limit 1
  ),
  self_scope as (
    select
...
comment on function public.get_listing_monitor_scope(uuid) is
  'Resolves customer-monitor visibility. Tenant admins and explicitly assigned platform support see their tenant; ordinary tenant members see self; unassigned users receive no customer-monitor scope.';
