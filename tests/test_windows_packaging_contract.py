...
def test_installer_is_stable_per_user_upgrade_and_uses_branded_icon() -> None:
    ...
    assert '#define InstalledIconName' not in INSTALLER
    assert 'Source: "{#IconFile}"; DestDir: "{app}\\icons"' not in INSTALLER
    assert 'IconFilename: "{app}\\{#MyAppExeName}"' in INSTALLER
