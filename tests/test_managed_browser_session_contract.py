...
def test_formal_top_level_browser_workspaces_cannot_overlap() -> None:
    gate = MANAGER.split("    def _assert_task_start_allowed(self) -> None:", 1)[1].split(
        "    def _assert_cached_browser_ready", 1
    )[0]
...
def test_login_failure_is_presented_as_user_action_not_browser_failure() -> None:
    ...
    assert '_HOT_STATES = {"READY", "LOGIN"}' in MANAGER


def test_single_start_click_contains_no_blocking_browser_io() -> None:
    start_single = MANAGER.split("    def _start_single", 1)[1].split(
        "    def _single_prepared", 1
    )[0]
    cached_gate = MANAGER.split("    def _assert_cached_browser_ready", 1)[1].split(
        "    def _recover_poisoned_locked", 1
    )[0]

    assert "ensure_ready(" not in start_single
    assert "probe_cdp_automation(" not in start_single
    assert "_cdp_instance_token(" not in start_single
    assert "ensure_async()" in cached_gate
    assert "probe_cdp_automation(" not in cached_gate
    assert "_cdp_instance_token(" not in cached_gate


def test_readonly_runner_starts_qprocess_without_duplicate_cdp_http_probe() -> None:
    start = READONLY.split("    def start(self, config: RunnerConfig", 1)[1].split(
        "    def stop", 1
    )[0]
    assert "_assert_makro_cdp_available" not in READONLY
    assert "urllib.request" not in READONLY
    assert "urlopen(" not in start
    assert "self._start_process(args)" in start


def test_browser_poll_network_io_is_dispatched_to_worker_thread() -> None:
    poll = MANAGER.split("    def _poll(self) -> None:", 1)[1].split(
        "    def _apply_endpoint_observation", 1
    )[0]
    assert "threading.Thread(" in poll
    assert "target=worker" in poll
    assert "self._cdp_instance_token()" in poll
    assert "self.endpoint_observed.emit(token)" in poll


def test_changed_startup_sources_still_compile() -> None:
    compile(MANAGER, str(ROOT / "gui" / "browser_session_manager.py"), "exec")
    compile(READONLY, str(ROOT / "gui" / "readonly_runner.py"), "exec")
