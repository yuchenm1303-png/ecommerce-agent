...
Write-Host "  [E2E 3/5] Verifying only the managed Edge CDP process was closed before Inno"
...
if (-not (Select-String -Path $UpdaterLog -Pattern "browser gate closing managed Edge" -Quiet)) { throw "Updater E2E did not exercise managed Edge ownership cleanup" }
...
Write-Host "  Updater E2E passed: managed Edge closed -> stubborn app -> surviving updater -> hidden Inno -> v$InstalledVersion -> real installed GUI"
exit 0
