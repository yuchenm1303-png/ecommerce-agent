...
& python -m PyInstaller `
    ...
    --distpath $EdgeDist `
    --workpath $EdgeWork `
    (Join-Path $Root "tests\windows_updater_fake_edge.py")
...
if (-not (Test-Path $FakeEdgeExe)) {
    throw "Updater E2E managed Edge executable missing"
}
...
if ($ExistingListener) {
    throw "Updater E2E reserved browser port $BrowserPort is already in use"
}
...
Write-Host "  [E2E 2/5] Frozen EcommerceAgent.exe hands off, stays alive, and is force-closed without killing updater.exe"
$ParentArgs = @(
    "--updater", $UpdaterExe,
...
Write-Host "  [E2E 3/5] Verifying updater closes only its managed Edge before Inno"
...
if ($Listener) {
    throw "Updater E2E managed Edge CDP port remained open after shutdown gate"
}

Write-Host "  [E2E 4/5] Waiting for updater survival + hidden Inno upgrade + installed VERSION verification"
...
if (-not (Select-String -Path $UpdaterLog -Pattern "closing managed Edge" -Quiet)) {
    throw "Updater E2E did not exercise managed Edge ownership cleanup"
}
if (-not (Select-String -Path $UpdaterLog -Pattern "browser gate closed managed Edge" -Quiet)) {
    throw "Updater E2E did not verify managed Edge termination"
}
if (-not (Select-String -Path $UpdaterLog -Pattern "running installer:.*\/VERYSILENT" -Quiet)) {
    throw "Updater E2E did not launch Inno in hidden /VERYSILENT mode"
}
if (-not (Select-String -Path $UpdaterLog -Pattern "native update panel shown" -Quiet)) {
    throw "Standalone updater did not create the continuous native progress panel"
}
...
Write-Host "  Updater E2E passed: stubborn app + managed Edge -> surviving updater -> hidden Inno -> v$InstalledVersion -> real installed GUI"
exit 0
